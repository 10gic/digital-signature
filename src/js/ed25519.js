////////////////////////////////////////////////////////////////////////////////
// From: https://github.com/paulmillr/noble-ed25519/blob/main/index.js
// I just added the following 2 functions: buildEd25519ExpandedPrivateKey, ed25519SignWithExpandedPrivKey
////////////////////////////////////////////////////////////////////////////////
...
// I just added the following 2 functions: buildEd25519ExpandedPrivateKey, ed25519SignWithExpandedPrivKey
////////////////////////////////////////////////////////////////////////////////
const buildEd25519ExpandedPrivateKey = async (priv) => {
    const hashed = await sha512a(toU8(priv, 32))
    const head = hashed.slice(0, 32); // slice creates a copy, unlike subarray
    head[0] &= 248; // Clamp bits: 0b1111_1000,
    head[31] &= 127; // 0b0111_1111,
    head[31] |= 64; // 0b0100_0000
    return concatB(head, hashed.slice(32, 64))
}

const ed25519SignWithExpandedPrivKey = async (msg, expandedPrivKey) => {
    const m = toU8(msg); // RFC8032 5.1.6: sign msg with key sync
    const expandedPrivKeyU8 = toU8(expandedPrivKey)
    const leftHalf = expandedPrivKeyU8.slice(0, 32)
    const prefix = expandedPrivKeyU8.slice(32, 64)
    const rBytes = await sha512a(prefix, m); // r = SHA512(dom2(F, C) || prefix || PH(M))

    const scalar = modL_LE(leftHalf) // modular division over curve order
    const point = G.mul(scalar); // public key point
    const pointBytes = point.toRawBytes(); // point serialized to Uint8Array
    const e = { pointBytes: pointBytes, scalar: scalar }

    return hashFinish(true, _sign(e, rBytes, m)); // gen R, k, S, then 64-byte signature
};

export { buildEd25519ExpandedPrivateKey, ed25519SignWithExpandedPrivKey }
